      ...
      <div onClick={(e: any) => e.stopPropagation()} className={styles.s2}>
        ...
        <div className={styles.s3}>
          ...
          <div className={styles.s6}>
            <button onClick={() => move(-1)} style={iconBtn}>
              <ChevronLeft size={18} />
            </button>
            <button onClick={() => move(1)} style={iconBtn}>
          ...
          <div className={styles.s8}>
            ...
            <button
              onClick={() => openCreate(toKey(view === "day" ? cursor : today))}
              style={createBtn}
            >
        ...
        <div className={styles.s16}>
          ...
          <div className={styles.s17}>
            ...
            <div className={styles.s18}>
              <div className={styles.s19}>Upcoming</div>
              {(() => {
                  ...
                  >
                    ...
                    <div>
                      ...
                      <div className="ui-text-caption ui-text-tertiary">
                        {MONTHS_SHORT[new Date(e.date + "T00:00").getMonth()]}{" "}
                        {new Date(e.date + "T00:00").getDate()} ·{" "}
            ...
            <div className={styles.s24}>
              <div className={styles.s25}>My calendars</div>
              {[
      ...
      <div className={styles.s34}>
        ...
        {cells.map((d: any, i: any) => {
          ...
          return (
            ...
            >
              {d.getDate()}
              {hasEvents && !isToday && <span className={styles.s37} />}
            </button>
          );
        })}
      </div>
        ...
        {cells.map((d: any, i: any) => {
          ...
          return (
            <div
              ...
              style={{
                borderRight:
                  i % 7 !== 6 ? "1px solid var(--color-border)" : undefined,
                background: isToday
                  ? "var(--color-primary-light)"
                  : inMonth
                    ? "transparent"
            ...
            >
              ...
              {evs.slice(0, 3).map((e: any) => (
                <button
                  ...
                  style={{
                    background: e.allDay ? colorOf(e) : `${colorOf(e)}20`,
              ...
              {evs.length > 3 && (
                <span className={styles.s48}>+{evs.length - 3} more</span>
              )}
            </div>
          );
        })}
        ...
        {days.map((d: any) => {
          ...
          return (
            <div key={toKey(d)} className={styles.s54}>
              ...
              <div
                style={{
                  fontSize: isMultiDay ? 22 : 28,
                  color: isToday ? "#fff" : "var(--color-text)",
                  background: isToday ? "var(--color-primary)" : "transparent",
                  width: isMultiDay ? 36 : 44,
                  height: isMultiDay ? 36 : 44,
                }}
                ...
                {evs.map((ev: any) => {
                  ...
                  return (
                    <button
                      ...
                      style={{ top: top, height: height - 2, background: c }}
                      className={styles.s65}
                    >
                      <div className={styles.s66}>{ev.title}</div>
                      <div className={styles.s67}>
                        {fmtTime(ev.time)} – {fmtTime(endTime(ev))}
                      </div>
                      {ev.location && (
                        <div className={styles.s68}>
                          <MapPin size={10} className={styles.s69} />{" "}
                          {ev.location}
                        </div>
                      )}
                    </button>
              ...
              {evs.map((e: any) => {
                ...
                return (
                  <button
                    key={e.id}
                    onClick={() => onPick(e)}
                    style={{ borderLeft: `4px solid ${colorOf(e)}` }}
                    className={styles.s78}
                  >
                    <div className="flex-1">
                      ...
                      <div className={styles.s80}>
                        {e.allDay
                          ? "All day"
                          : `${fmtTime(e.time)} – ${fmtTime(endTime(e))}`}
                        {e.location && (
                          <>
                            {" "}
                            · <MapPin size={11} className={styles.s69} />{" "}
                            {e.location}
                          </>
                        )}
                      </div>
                    ...
                    <div className="ui-flex ui-items-center ui-gap-1">
                      {e.meetingCode && (
                        <Video size={15} className="ui-text-primary" />
                      )}
                      {e.recurrence && e.recurrence !== "none" && (
                        <Repeat size={13} className="ui-text-tertiary" />
                      )}
                      {attendees.length > 0 && (
      ...
      <div className={styles.s96}>
        ...
        {e.meetingCode && (
          <div className={styles.s101}>
            ...
            <div className={styles.s104}>
              <button onClick={onJoin} className={styles.s105}>
                <Video size={14} /> Join meeting
              </button>
              <button
                onClick={() =>
                  navigator.clipboard?.writeText(
                    `connect.meet/${e.meetingCode}`,
                  )
                }
                title="Copy link"
        ...
        <div>
          <div className={styles.s107}>
            <Users size={14} /> {attendees.length} guest
          ...
          <div className={styles.s108}>
            ...
            {attendees.length === 0 && (
              <span className={styles.s20}>No guests</span>
            )}
          </div>
        </div>
      </div>
  ...
  return (
    <div className={styles.s89}>
              ...
              <div className={styles.s127}>
                {EVENT_COLORS.map((c: any) => (
                  <button
                    ...
                    className={styles.s128}
                  >
                    {form.color === c.value && (
                      <Check size={14} className={styles.s129} />
                    )}
                  </button>
                ))}
              </div>
      ...
      <div className={styles.s138}>
        <button onClick={onCancel} className={styles.s139}>
          Cancel
        </button>
        <button
          onClick={onSubmit}
