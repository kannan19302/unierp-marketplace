    ...
    >
      <span
        style={{
          width: size,
          height: size,
          background: avatarColor(member.id || member.name),
          fontSize: size * 0.36,
        }}
        className={styles.s3}
      >
...
function Unread({ n }: { n: number }) {
  if (!n) return null;
  return <span className={styles.s7}>{n > 99 ? "99+" : n}</span>;
}
...
function DateDivider({ ts }: { ts: number }) {
  return (
    <div className={styles.s8}>
      ...
      <span className={styles.s10}>{formatDateDivider(ts)}</span>
      <div className={styles.s9} />
    </div>
  );
}
...
export default function ConnectPage() {
  ...
  const [typingUsers, setTypingUsers] = useState<Map<string, number>>(
    new Map(),
  ); // userId -> last-typing timestamp, per active channel
  ...
  const switcherDebounceRef = useRef<ReturnType<typeof setTimeout> | null>(
    null,
  );
  ...
  const [polls, setPolls] = useState<{
    [channelId: string]: import("./connectData").ConnectPoll[];
  }>({});
  ...
  const [scheduledMsgs, setScheduledMsgs] = useState<
    import("./connectData").ScheduledMessage[]
  >([]);
  ...
  useEffect(() => {
    loadWorkspace();
    loadBookmarks();
  }, [loadWorkspace, loadBookmarks]);

  // Deep-link support: the profile hover card's "Message" / "Notes to Self"
  // buttons navigate here as /connect?dm=<channelId>. Runs once at mount and
  // wins over loadWorkspace's own "pick the first channel" fallback above,
  // since that only applies when activeId is still null.
  useEffect(() => {
  ...
  useEffect(() => {
    const t = setInterval(() => {
      loadWorkspace();
      if (activeId) api.markRead(activeId).catch(() => {});
    }, 15000);
    return () => clearInterval(t);
    ...
    socket.on(
      ...
      (payload: {
        ...
        setWs((prev: any) => {
          ...
          const mapped =
            (payload.presence as Presence | undefined) ??
  ...
  useEffect(() => {
    ...
    return () => {
      socket.emit("leave:channel", { channelId: activeId });
    };
  ...
  useEffect(() => {
    ...
    const bump = () => {
      ...
      if (autoAwayRef.current && ws?.me.presence === "AWAY") {
  ...
  useEffect(() => {
    ...
    if (switcher === null || !switcher.trim()) {
      setSwitcherMessages([]);
      return;
    }
    switcherDebounceRef.current = setTimeout(async () => {
      try {
        setSwitcherMessages(await api.search(switcher.trim()));
      } catch {
        setSwitcherMessages([]);
      }
    }, 300);
    return () => {
      if (switcherDebounceRef.current)
        clearTimeout(switcherDebounceRef.current);
    };
  }, [switcher]);
  ...
  const jumpToMessage = async (channelId: string, messageId: string) => {
    if (activeId !== channelId) {
      ...
      setPinnedView(false);
      try {
        setMessages(await api.messages(channelId));
      } catch {
        /* keep prior */
      }
    }
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (socketRef.current)
        socketRef.current.emit("leave:channel", { channelId: activeId }); // no-op placeholder to keep room fresh; server also broadcasts chat:message
    } catch (e) {
            ...
            prev.map((a: any) =>
              a.localId === localId
                ? {
                    ...
                    status: "done",
                    progress: 100,
                    documentId: res.documentId ?? res.attachment?.id,
                    url: attachmentUrl(res.attachment?.url),
                  }
                : a,
            ),
          );
  ...
  const handleThreadSend = async () => {
    ...
    const content = threadComposer;
    setThreadComposer("");
    try {
      await api.send(activeId, { content, parentId: threadParent });
      await loadMessages(activeId);
    } catch (e) {
      alert(e instanceof Error ? e.message : "Reply failed");
      setThreadComposer(content);
    }
  };
  ...
  const saveEdit = async () => {
    if (!editingId) return;
    try {
      replaceMsg(await api.edit(editingId, editText));
    } catch (e) {
  ...
  const del = async (id: string) => {
    try {
      replaceMsg(await api.remove(id));
    } catch (e) {
  ...
  const startMeeting = async () => {
    ...
    try {
      const m = await api.createMeeting({
        title: `${activeConv?.name ?? "Connect"} call`,
      ...
      alert(e instanceof Error ? e.message : "Could not start meeting");
    }
  };
  const joinMeeting = () => {
  ...
  const sendMeetingChat = () => {
    ...
    setMeetingState((s: any) => ({
      ...
      meetingChat: [
        ...s.meetingChat,
        { author: me.name, text: s.meetingChatDraft, ts: Date.now() },
      ],
      meetingChatDraft: "",
    }));
  };
  ...
  const addEvent = async (ev: Parameters<typeof api.createEvent>[0]) => {
    try {
      ...
      alert(e instanceof Error ? e.message : "Could not add event");
    }
  };
  ...
  const createGroup = async () => {
    ...
    try {
      const conv = await api.createGroup(name, people.selected);
      await loadWorkspace();
      setActiveId(conv.id);
      setPeople(null);
    } catch (e) {
  ...
  const newChannel = async (spaceId?: string) => {
    const name = prompt("New channel name (e.g. design):");
    if (!name) return;
    try {
      const c = await api.createChannel({ name, spaceId });
      await loadWorkspace();
      setActiveId(c.id);
    } catch (e) {
  ...
  const newSpace = async () => {
    ...
    try {
      ...
      alert(e instanceof Error ? e.message : "Could not create space");
    }
  };
  ...
  const openManageChannel = async () => {
    ...
    setManageName(activeConv.name);
    setManageTopic(activeConv.topic ?? "");
    setManageTab("general");
    setManageChannelOpen(true);
    setManageMembers(null);
    setMemberSearch("");
    try {
      setManageMembers(await api.channelMembers(activeConv.id));
    } catch {
      setManageMembers([]);
    } // gap-fill endpoint may not be deployed everywhere yet — degrade to empty roster
  };
  const saveChannelName = async () => {
    if (
      !activeConv ||
      !manageName.trim() ||
      manageName.trim() === activeConv.name
    )
      return;
    try {
      await api.updateChannel(activeConv.id, { name: manageName.trim() });
      await loadWorkspace();
      toast.success("Saved");
    } catch (e) {
      toast.error(
  ...
  const saveChannelTopic = async () => {
    ...
    try {
      await api.updateChannel(activeConv.id, { topic: manageTopic });
      await loadWorkspace();
      toast.success("Saved");
    } catch (e) {
      toast.error(
  ...
  const openBrowseChannels = async () => {
    ...
    setBrowseSearch("");
    try {
      setBrowseList(await api.browseChannels());
    } catch (e) {
      setBrowseErr(e instanceof Error ? e.message : "Could not load channels");
    }
  };
  ...
  /* ── Forward dialog (spec §7a) ── */
  const openForward = (m: ConnectMessage) => {
    setForwardMsg(m);
    ...
    <div className={styles.s11}>
      <button onClick={() => toggleSection(id)} className={styles.s12}>
        <ChevronRight
          size={12}
          style={{
            transform: collapsedSections.has(id)
              ? "rotate(0)"
              : "rotate(90deg)",
          }}
    ...
    return (
      <RouteGuard permission="communication.read">
        <div className={styles.s18}>
          <RefreshCw className="animate-spin" size={26} /> Loading Connect...
        </div>
      </RouteGuard>
    );
  }
      ...
      <div className={styles.s19} data-active-channel={activeId || ""}>
        ...
        <div className={styles.s20}>
          ...
          <div className={styles.s23}>
                ...
                <>
                  ...
                  <div className={styles.s26}>
                    <div className={styles.s27}>
                      ...
                      <button
                        onClick={() => {
                          setMyPresence(me.presence, statusDraft);
                          setPresenceOpen(false);
                        }}
            ...
            <button onClick={openCalendar} style={tbtn()}>
        ...
        <div className={styles.s39}>
          ...
          <div className={styles.s40}>
            ...
            <div className={styles.s44}>
                  ...
                  {!collapsedSections.has(sp.id) &&
                    channelsBySpace(sp.id).map((c: any) => (
                      <button
                        key={c.id}
                        onClick={() => switchConv(c.id)}
                        style={pill(activeId === c.id, mutedConvs.has(c.id))}
                      >
                        <Hash size={14} />
                        <span
                          style={{
                            fontWeight: unreadOf(c) ? 700 : undefined,
                            color: unreadOf(c)
                              ? "var(--color-text)"
                              : undefined,
                          }}
                        ...
                        {mutedConvs.has(c.id) && (
                          <BellOff size={11} className="ui-text-tertiary" />
                        )}
                        <Unread n={unreadOf(c)} />
                      </button>
                    ))}
              ...
              {ungroupedChannels.length > 0 && (
                <div>
                  ...
                  >
                    <Search size={13} /> Browse channels
                  </button>
                </div>
              )}
              ...
              {/* Direct messages */}
              <div>
                {sectionHeader("dms", "Direct messages", () =>
                  setPeople({ mode: "dm", selected: [], search: "" }),
                )}
                  ...
                  directs.map((c: any) => {
                    ...
                    return (
                      <button
                        ...
                        onClick={() => switchConv(c.id)}
                        style={{
                          ...pill(activeId === c.id, mutedConvs.has(c.id)),
                        }}
                        className={styles.s48}
                      >
                        {other ? (
                          <Avatar member={other} size={24} showPresence />
                        ) : (
                          <Users size={18} />
                        )}
                        <div className="flex-1 overflow-hidden">
                          <span
                            style={{
                              ...
                              color: unread ? "var(--color-text)" : undefined,
                            }}
                ...
                <button
                  onClick={() =>
                    setPeople({ mode: "group", selected: [], search: "" })
                  }
                  style={{ ...pill(false) }}
                  className={styles.s47}
                >
            ...
            <button
              ...
              className={styles.s51}
            >
              <span className={styles.s52} />
              <span className={styles.s53}>
            ...
            {!activeConv ? (
              <div className={styles.s59}>
                ...
                <div className={styles.s62}>
                  ...
                  <button onClick={() => newChannel()} style={tbtn()}>
              ...
              <>
                ...
                <div className={styles.s63}>
                  <div className="flex-1 overflow-hidden">
                    <div className={styles.s64}>
                      ...
                      {activeConv.kind === "CHANNEL" && (
                        <Globe size={13} className="ui-text-tertiary" />
                      )}
                      <button
                        onClick={() => toggleStar(activeConv.id)}
                        style={{
                          color: starredConvs.has(activeConv.id)
                            ? "#f6bf26"
                            : "var(--color-text-tertiary)",
                        }}
                        className={styles.s66}
                      >
                        <Star
                          size={14}
                          fill={
                            starredConvs.has(activeConv.id) ? "#f6bf26" : "none"
                          }
                        />
                      </button>
                    </div>
                    {activeConv.topic && (
                      <p className={styles.s67}>{activeConv.topic}</p>
                    )}
                  </div>
                  <div className="ui-flex ui-items-center ui-gap-1">
                          ...
                          {!searchLoading &&
                            searchResults?.map((r: any) => (
                              <button
                                key={r.messageId}
                                onClick={() => {
                                  jumpToMessage(r.channelId, r.messageId);
                                  setConvSearch("");
                                }}
                                className={styles.s74}
                              >
                                <div className={styles.s75}>
                                  <strong className={styles.s76}>
                                    {r.authorName}
                                  </strong>
                                  <span className={styles.s77}>
                                    {formatDateSmart(r.ts)}
                                  </span>
                                </div>
                                <div className={styles.s78}>
                                  {highlightSnippet(r.snippet, convSearch)}
                                </div>
                              </button>
                            ))}
                        </div>
                    ...
                    {pinned.length > 0 && (
                      <button
                        onClick={() => setPinnedView(!pinnedView)}
                        className={styles.s79}
                      >
                        <Pin size={13} className="ui-text-warning" />{" "}
                        {pinned.length}
                      </button>
                    )}
                    ...
                    <button
                      onClick={startMeeting}
                      title="Start meeting"
                      className={styles.s81}
                    >
                      <Video size={16} />
                    </button>
                    <button
                      onClick={() => {
                        setSavedMessagesOpen(!savedMessagesOpen);
                        setChannelInfo(false);
                        setThreadParent(null);
                      }}
                      title="Saved messages"
                      style={{
                        color: savedMessagesOpen
                          ? "var(--color-primary)"
                          : "var(--color-text-tertiary)",
                      }}
                      className={styles.s82}
                ...
                {/* ─ Channel tabs ─ */}
                {activeConv.kind === "CHANNEL" && activeConv.id && (
                  <ChannelTabs channelId={activeConv.id} />
                )}

                {/* ─ Pinned messages bar ─ */}
                {pinned.length > 0 && !pinnedView && (
                  <button
                    onClick={() => setPinnedView(true)}
                    className={styles.s83}
                  >
                    <Pin size={12} className="ui-text-warning" />
                    <span className={styles.s46}>
                      <strong>{memberById(pinned[0]!.authorId).name}:</strong>{" "}
                      {pinned[0]!.content}
                    </span>
                    {pinned.length > 1 && (
                      <span className={styles.s77}>{pinned.length} pinned</span>
                    )}
                  </button>
                )}
                ...
                <div ref={feedRef} className={styles.s84}>
                      ...
                      {group.messages.map((m: any, i: any) => {
                        ...
                        return (
                          <MessageRow
                            ...
                            me={me.id}
                            replyCount={repliesOf(m.id).length}
                            compact={!!compact}
                  ...
                  {topLevel.length === 0 && (
                    <div className={styles.s85}>
                      {convSearch ? (
                        <>
                          <Search size={32} className={styles.s60} />
                          <span className={styles.s86}>
                            No messages match &quot;{convSearch}&quot;
                          </span>
                        </>
                      ) : (
                  ...
                  {mentionMatches.length > 0 && (
                    <div className={styles.s91}>
                      ...
                      {mentionMatches.map((m: any) => (
                        ...
                        >
                          <Avatar member={m} size={24} showPresence />{" "}
                          <span className={styles.s89}>{m.name}</span>
                  ...
                  <div className={styles.s99}>
                    ...
                    >
                      ...
                      <button
                        onClick={() =>
                          setEmojiFor(
                            emojiFor === "composer" ? null : "composer",
                          )
                        }
                      ...
                      <button
                        ...
                        style={{
                          background:
                            composer.trim() || staged.length
                              ? "var(--color-primary)"
                              : "var(--color-border)",
                        }}
                        className={styles.s103}
                      >
                        <Send size={16} />
                      </button>
                    </div>
                    {showSchedulePicker && (
                      ...
                      >
                        ...
                        <button
                          onClick={async () => {
                            if (!scheduleAt || !composer.trim() || !activeId)
                              return;
                            try {
              ...
              return (
                <div className={styles.s105}>
                  <div className={styles.s106}>
                    <h3 className={styles.s107}>
                      <BookmarkCheck size={15} /> Saved messages
                    </h3>
                    <button
                      onClick={() => setSavedMessagesOpen(false)}
                      className={styles.s108}
                    >
                      <X size={16} />
                    </button>
                  </div>
                      ...
                      savedMessages.map((m: any) => (
                        <div key={m.id} className={styles.s111}>
                          <div className={styles.s112}>
                            ...
                            <span className={styles.s114}>
                              {formatDateSmart(m.ts)}
                            </span>
                          </div>
                          ...
                          <div className={styles.s116}>
                            <button
                              onClick={() =>
                                jumpToMessage(m.conversationId, m.id)
                              }
                              className={styles.s117}
                            >
                              <Eye size={12} /> Go to message
                            </button>
                            <button
                              onClick={() => toggleBookmark(m.id)}
                              className={styles.s118}
                            >
              ...
              return (
                <div className={styles.s105}>
                  ...
                  <div className={styles.s119}>
                    <ThreadMsg
                      m={parent}
                      author={memberById(parent.authorId)}
                    />
                  ...
                  <div className={styles.s121}>
                    <textarea
                      ...
                      placeholder="Reply..."
                      rows={1}
                      className={styles.s122}
                    />
                    <button
                      onClick={handleThreadSend}
                      disabled={!threadComposer.trim()}
                      style={{
                        background: threadComposer.trim()
                          ? "var(--color-primary)"
                          : "var(--color-border)",
                      }}
                      className={styles.s123}
                    >
                      <Send size={16} />
                    </button>
                  </div>
              ...
              <div className={styles.s126}>
                <div className={styles.s127}>
                  <div className={styles.s128}>
                    {activeConv.kind === "CHANNEL" ? (
                      <Hash size={28} className="ui-text-primary" />
                    ) : (
                  ...
                  {activeConv.topic && (
                    <p className={styles.s130}>{activeConv.topic}</p>
                  )}
                </div>
                ...
                <div>
                  <div className={styles.s133}>
                    <span className={styles.s134}>
                      Members ({headerMembers.length})
                    </span>
                  ...
                  {headerMembers.map((m: any) => (
                    <button
                      ...
                      className={styles.s135}
                    >
                      <Avatar member={m} size={28} showPresence />
                      <div className="flex-1 overflow-hidden">
                        <div className={styles.s136}>
                          {m.name}{" "}
                          {m.id === me.id && (
                            <span className="ui-text-caption ui-text-tertiary">
                              (you)
          ...
          {pinnedView && activeConv && (
            <div className={styles.s144}>
              <div className={styles.s106}>
                <h3 className={styles.s107}>
                  <Pin size={14} className="ui-text-warning" /> Pinned messages
                </h3>
                <button
                ...
                {pinned.map((m: any) => (
                  <div key={m.id} className={styles.s146}>
                    <div className={styles.s147}>
                      ...
                      <span className={styles.s148}>
                        {memberById(m.authorId).name}
                      </span>
                      <span className={styles.s94}>
                        {formatDateSmart(m.ts)}
                      </span>
                    </div>
                    <div className={styles.s149}>{m.content}</div>
                    <button onClick={() => pin(m.id)} className={styles.s150}>
                      Unpin
                    </button>
                  </div>
                ))}
                {pinned.length === 0 && (
                  <p className={styles.s151}>No pinned messages</p>
                )}
              </div>
            </div>
          )}
                  ...
                  <div className={styles.s154}>
                    ...
                    <div className={styles.s164}>
                      ...
                      <div className={styles.s168}>
                        ...
                        <span className={styles.s169}>
                          {participants.length} in this call
                        </span>
                      </div>
                      <button onClick={joinMeeting} className={styles.s170}>
                        Join now
                      </button>
                      <button
                        onClick={() => {
                        ...
                        className={styles.s171}
                      >
                        <Copy size={15} /> Copy joining info
                      </button>
                      <button
                        onClick={() => {
                          setActiveMeeting(null);
            ...
            return (
              <div className={styles.s173}>
                ...
                <div className={styles.s174}>
                  ...
                  <span className={styles.s177}>
                    {fmtElapsed(ms.elapsedSec)}
                  </span>
                  {ms.recording && (
                  ...
                  <div className={styles.s182}>
                    ...
                    {ms.showCaptions && (
                      <div className={styles.s186}>
                        <p className={styles.s187}>
                          <span className={styles.s175}>{me.name}: </span>
                          Captions will appear here when speaking...
                        </p>
                      </div>
                    )}
                        ...
                        gridTemplateColumns:
                          ms.layout === "gallery"
                            ...
                            : ms.layout === "spotlight"
                              ...
                              : "1fr 280px",
                        gridAutoRows:
                          ms.layout === "spotlight" ? "1fr" : undefined,
                      }}
                      className={styles.s188}
                    >
                      {(ms.layout === "spotlight"
                        ? [participants[0]!]
                        : participants
                        ...
                        <div
                          ...
                          style={{
                            border:
                              p.id === me.id && ms.handRaised
                                ? "2px solid #f6bf26"
                                : "2px solid transparent",
                          }}
                          className={styles.s189}
                        >
                          {/* Simulated video — gradient bg + avatar */}
                          <div
                            style={{
                              background: `linear-gradient(${135 + i * 30}deg, #1a1a2e, #16213e, #0f3460)`,
                              opacity: p.id === me.id && !ms.camOn ? 0 : 0.8,
                            }}
                            className={styles.s190}
                          />
                          <Avatar
                            member={p}
                            size={
                              ms.layout === "spotlight" && i === 0 ? 96 : 56
                            }
                          />
                          ...
                          <div className={styles.s191}>
                            {p.id === me.id && !ms.micOn && (
                              <MicOff size={12} className={styles.s192} />
                            )}
                            <span className={styles.s193}>
                              {p.name}
                              {p.id === me.id ? " (You)" : ""}
                            </span>
                            {p.id === me.id && ms.handRaised && (
                              <span className={styles.s86}>✋</span>
                            )}
                          </div>

                          {/* Pin / menu on hover */}
                          <div className={styles.s194}>
                            <button className={styles.s195} title="More">
                    ...
                    {ms.layout === "spotlight" && participants.length > 1 && (
                      <div className={styles.s196}>
                        <div
                          style={{ opacity: ms.camOn ? 0.8 : 0 }}
                          className={styles.s197}
                        />
                        <Avatar member={me} size={40} />
                        <div className={styles.s198}>
                          {!ms.micOn && (
                            <MicOff size={10} className={styles.s192} />
                          )}
                    ...
                    <div className={styles.s200}>
                      <div className={styles.s201}>
                        <button
                          ...
                          style={{
                            color: ms.showParticipants ? "#8ab4f8" : "#9aa0a6",
                            borderBottom: ms.showParticipants
                              ? "2px solid #8ab4f8"
                              : "2px solid transparent",
                          }}
                          className={styles.s202}
                        >
                        ...
                        <button
                          ...
                          style={{
                            color: ms.showChat ? "#8ab4f8" : "#9aa0a6",
                            borderBottom: ms.showChat
                              ? "2px solid #8ab4f8"
                              : "2px solid transparent",
                          }}
                          className={styles.s202}
                        ...
                        <>
                          <div className={styles.s119}>
                            {ms.meetingChat.length === 0 && (
                              <div className={styles.s214}>
                                <MessageCircle
                                  size={32}
                                  className={styles.s215}
                                />
                                <p className={styles.s216}>
                                  Messages can only be seen by people in the
                                  call and are deleted when the call ends.
                                </p>
                              </div>
                            )}
                          ...
                          <div className={styles.s221}>
                            <input
                              ...
                              className={styles.s222}
                            />
                            <button
                              onClick={sendMeetingChat}
                              disabled={!ms.meetingChatDraft.trim()}
                              style={{
                                background: ms.meetingChatDraft.trim()
                                  ? "#8ab4f8"
                                  : "transparent",
                                color: ms.meetingChatDraft.trim()
                                  ? "#202124"
                                  : "#5f6368",
                              }}
                              className={styles.s223}
                            >
                              <Send size={16} />
                            </button>
                          </div>
                ...
                <div className={styles.s224}>
                  ...
                  <div className={styles.s21}>
                    ...
                    {meetCtrl(
                      <Hand
                        size={20}
                        style={ms.handRaised ? { color: "#f6bf26" } : undefined}
                      />,
                      ...
                      <div className={styles.s230}>
                        {[
                          ...
                          <button
                            ...
                            onClick={opt.action}
                            style={{
                              color: opt.active ? "#8ab4f8" : "#e8eaed",
                            }}
                            className={styles.s231}
                          >
                            {opt.icon} {opt.label}
                          </button>
                        ))}
                  ...
                  <div className={styles.s233}>
                    <button
                      ...
                      title="Chat"
                      style={{
                        background: ms.showChat
                          ? "rgba(138,180,248,.2)"
                          : "transparent",
                        color: ms.showChat ? "#8ab4f8" : "#e8eaed",
                      }}
                      className={styles.s234}
                    >
                    ...
                    <button
                      ...
                      title="People"
                      style={{
                        background: ms.showParticipants
                          ? "rgba(138,180,248,.2)"
                          : "transparent",
                        color: ms.showParticipants ? "#8ab4f8" : "#e8eaed",
                      }}
                      className={styles.s234}
                    >
                      <Users size={18} />
                    ...
                    <button
                      ...
                      title="Meeting recap"
                      style={{
                        background: showMeetingRecap
                          ? "rgba(138,180,248,.2)"
                          : "transparent",
                        color: showMeetingRecap ? "#8ab4f8" : "#e8eaed",
                      }}
                      className={styles.s234}
                    >
                      <FileText size={18} />
          ...
          <Modal onClose={() => setPeople(null)} width={480}>
            <div className={styles.s236}>
              <div className="ui-flex-between">
                <h2 className={styles.s22}>
                  {people.mode === "dm" ? "New message" : "New group"}
                </h2>
                <button
                      ...
                      >
                        {people.mode === "group" &&
                          (checked ? (
                            ...
                            <Square size={16} className="ui-text-tertiary" />
                          ))}
              ...
              {people.mode === "group" && (
                <button
                  onClick={createGroup}
                  disabled={people.selected.length === 0}
                  style={tbtn({
                    background: "var(--color-primary)",
                    color: "#fff",
                    border: "none",
                    justifyContent: "center",
                    opacity: people.selected.length ? 1 : 0.6,
                    width: "100%",
                  })}
                >
                  Create group ({people.selected.length})
                </button>
              )}
            </div>
          </Modal>
            ...
            <div className={styles.s245}>
              ...
              {!switcher && (
                <div className={styles.s250}>
                  <span className="font-semibold">Tip:</span> Use Ctrl+K anytime
                  to open this search
                </div>
              )}
              <div className={styles.s251}>
                        ...
                        (r: any) =>
                          r.type === "conv" && (
                            <button
                              ...
                              className={styles.s253}
                            >
                              {r.c.kind === "CHANNEL" ? (
                                <Hash size={16} className="ui-text-muted" />
                              ) : r.c.kind === "GROUP" ? (
                                <Users size={16} className="ui-text-muted" />
                              ) : (
                                <MessageSquare
                                  size={16}
                              ...
                              {r.c.topic && (
                                <span className={styles.s254}>{r.c.topic}</span>
                              )}
                              {(r.c.unreadCount ?? 0) > 0 && (
                                <Unread n={r.c.unreadCount ?? 0} />
                              )}
                            </button>
                          ),
                      )}
                        ...
                        (r: any) =>
                          r.type === "person" && (
                            <button
                              ...
                              className={styles.s253}
                            >
                              <Avatar member={r.d} size={28} showPresence />{" "}
                              <span className={styles.s86}>{r.d.name}</span>
                      ...
                      >
                        ...
                        <div className="flex-1 overflow-hidden">
                          <div className={styles.s257}>
                            ...
                            <span className={styles.s258}>
                              {formatDateSmart(r.ts)}
                            </span>
                          </div>
                          <div className={styles.s259}>
                            {highlightSnippet(r.snippet, switcher)}
                          </div>
                        </div>
                      </button>
                    ))}
                ...
                {switcher &&
                  switcherResults.length === 0 &&
                  switcherMessages.length === 0 && (
                    <p className={styles.s260}>No results found</p>
                  )}
              </div>
            </div>
          </Modal>
            ...
            <div className={styles.s261}>
              ...
              <div className={styles.s56}>
                ...
                <div className={styles.s263}>
                  <PresenceDot
                    presence={profileCard.presence}
                    size={8}
                    border="transparent"
                  />
                  <span className={styles.s264}>
                    {PRESENCE_META[profileCard.presence].label}
                  </span>
                </div>
                {profileCard.statusText && (
                  <p className={styles.s265}>{profileCard.statusText}</p>
                )}
                {(profileCard.designation || profileCard.department) && (
                  <p className={styles.s266}>
                    {profileCard.designation || "Staff"}{" "}
              ...
              <div className={styles.s62}>
                <button
                  onClick={() => {
                    setProfileCard(null);
                    startDM(profileCard.id);
                  }}
                  style={{ ...tbtn({ justifyContent: "center" }) }}
                  className={styles.s267}
                >
        ...
        <Drawer
          ...
          onClose={() => setManageChannelOpen(false)}
          title={activeConv ? `Manage #${activeConv.name}` : "Manage channel"}
          width={440}
        >
          <ProtectedComponent
            permission="communication.channel.manage"
            fallback={
              <p className={styles.s278}>
            ...
            <div className={styles.s261}>
              <Tabs
                tabs={[
                  ...
                  {
                    key: "members",
                    label: `Members (${manageMembers?.length ?? 0})`,
                  },
                ]}
                value={manageTab}
                            ...
                            <div className={styles.s281}>
                              ...
                              {results.map((d: any) => (
                                <button
                                  ...
                                  className={styles.s283}
                                >
                                  <Avatar member={d} size={24} />{" "}
                                  <span className={styles.s89}>{d.name}</span>
                                </button>
                              ))}
                            </div>
                          );
                            ...
                            <ProtectedComponent permission="communication.channel.member.manage">
                              {!soleOwner && (
                                <button
                                  ...
                                  aria-label={`Remove ${person.name} from channel`}
                                  className={styles.s286}
                                >
                                  <X size={15} />
                                </button>
                              )}
          ...
          <div className={styles.s236}>
            ...
            {browseErr && (
              <div className={styles.s287}>
                ...
                <button onClick={openBrowseChannels} className={styles.s288}>
                  Retry
                </button>
              </div>
            )}
                    ...
                    {filtered.map((c: any) => (
                      <div key={c.id} className={styles.s290}>
                        ...
                        <div className="flex-1 overflow-hidden">
                          ...
                          {c.topic && (
                            <div className={styles.s259}>{c.topic}</div>
                          )}
                          <div className="ui-text-caption ui-text-tertiary">
                            {c.memberCount} member
                            {c.memberCount === 1 ? "" : "s"} · Public
                          </div>
                        </div>
                        <Button
                          variant="secondary"
        ...
        <UiModal
          ...
          footer={
            <>
              <Button variant="secondary" onClick={() => setForwardMsg(null)}>
                Cancel
              </Button>
              <Button
                variant="primary"
            ...
            <div className={styles.s236}>
              <div className={styles.s291}>
                <div className={styles.s292}>
                  ...
                  <strong className={styles.s89}>
                    {memberById(forwardMsg.authorId).name}
                  </strong>
                  <span className="ui-text-caption ui-text-tertiary">
                    {formatDateSmart(forwardMsg.ts)}
                  </span>
                </div>
        ...
        {showEmojiManager && (
          <EmojiManager onClose={() => setShowEmojiManager(false)} />
        )}
        {showMeetingRecap && activeMeeting && (
          <MeetingRecap
            meetingId={activeMeeting.id}
            onClose={() => setShowMeetingRecap(false)}
          />
        )}
        {showTemplateDialog && (
          ...
          >
            ...
            >
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <button
                  onClick={() => setShowReminderCreator(false)}
                  style={{
                    flex: 1,
                    background: "none",
                    border: "1px solid var(--color-border)",
  ...
  return (
    <div className={styles.s296}>
      ...
      <div className={styles.s297}>
        ...
        <div className={styles.s299}>
          {m.deleted ? (
            <em className="ui-text-tertiary">message deleted</em>
          ) : (
            renderContent(m.content)
          )}
        </div>
  ...
  return (
    <div
      style={{
        ...
        minWidth: loaded ? undefined : 160,
      }}
      className={styles.s302}
    >
...
function MessageRow(p: RowProps) {
  ...
  if (m.kind === "SYSTEM") {
    return (
      <div className={styles.s304}>
        ...
        <span className={styles.s305}>
          <strong className={styles.s76}>{author.name}</strong> {m.content}
        </span>
        {m.meetingId && (
          <button onClick={p.onJoinMeeting} className={styles.s306}>
            Join
          </button>
        )}
      </div>
  ...
  return (
    <div
      ...
      style={{
        gap: p.compact ? 0 : 10,
        padding: p.compact ? "1px 8px 1px 54px" : "6px 8px",
        background: p.flashing
          ? "var(--color-primary-light)"
          : hover
            ? "var(--color-bg-hover)"
            : "transparent",
        transition: p.flashing ? "background 1.2s ease-out" : "background .1s",
      }}
      className={styles.s307}
    >
      {!p.compact && (
        <Avatar
          member={author}
          size={36}
          showPresence
          onClick={() => p.onProfileClick(author)}
        />
      )}
      <div className="flex-1 overflow-hidden">
        {!p.compact && (
          <div className={styles.s64}>
            ...
            {m.editedTs && !m.deleted && (
              <span className={styles.s114}>(edited)</span>
            )}
        ...
        {m.deleted ? (
          ...
          <div className={styles.s310}>
            <input
              ...
              autoFocus
              className={styles.s311}
            />
            <button onClick={p.onSaveEdit} className={styles.s312}>
              Save
            </button>
            <button onClick={p.onCancelEdit} className={styles.s313}>
              Cancel
            </button>
          </div>
        ) : forwardedInfo ? (
          <div>
            <div className={styles.s314}>
              ...
              <div className={styles.s316}>
                <strong className={styles.s89}>
                  {forwardedInfo.originAuthor}
                </strong>
                <span className="ui-text-caption ui-text-tertiary">
                  {formatDateSmart(forwardedInfo.originTs)}
                </span>
              </div>
              <div className={styles.s299}>
                {renderContent(forwardedInfo.body)}
              </div>
            </div>
          </div>
        ) : (
        ...
        {!m.deleted && (m as any).viewOnce && (
          <EphemeralBadge message={m} onView={() => {}} />
        )}
        {!m.deleted && !m.parentId && (m as any).pollId && (
          <PollBlockInline
            pollId={(m as any).pollId}
              ...
              {m.attachments
                ...
                .map((a: any) => (
                  <a
                    ...
                    className={styles.s321}
                  >
                    {a.url ? (
                      <FileText size={18} className={styles.s322} />
                    ) : (
                      <Spinner size="sm" />
                    )}
                    <div className="flex-1 overflow-hidden">
          ...
          <div className={styles.s324}>
            ...
            {p.replyCount > 0 && (
              <button onClick={p.onOpenThread} className={styles.s326}>
                <Reply size={12} /> {p.replyCount} repl
      ...
      {(hover || p.emojiOpen) && !m.deleted && !p.editing && (
        <div className={styles.s328}>
          <IconBtn title="React" onClick={p.onEmojiToggle}>
            <Smile size={15} />
          </IconBtn>
          <IconBtn title="Reply" onClick={p.onOpenThread}>
            <Reply size={15} />
          </IconBtn>
          <IconBtn title="Forward" onClick={p.onForward}>
            <Forward size={15} />
          </IconBtn>
          <IconBtn title={m.pinned ? "Unpin" : "Pin"} onClick={p.onPin}>
            <Pin size={15} />
          </IconBtn>
          <IconBtn
          ...
          {m.authorId === p.me && (
            <IconBtn title="Edit" onClick={p.onEdit}>
              <Pencil size={15} />
            </IconBtn>
          )}
          {m.authorId === p.me && (
            <IconBtn title="Delete" onClick={p.onDelete}>
              <Trash2 size={15} />
            </IconBtn>
          )}
          {p.emojiOpen && (
...
function IconBtn({
  ...
  return (
    <button onClick={onClick} title={title} className={styles.s330}>
      {children}
    </button>
  );
}
    ...
    return (
      <span className={styles.s331}>
        <span className={styles.s64}>
          {isImageMime(a.mime) ? (
            <Image size={14} className="ui-text-primary" />
          ) : (
            <FileText size={14} className="ui-text-muted" />
          )}
          <span className={styles.s46}>{a.name}</span>
          <button
            disabled
            aria-label={`Remove ${a.name}`}
            className={styles.s332}
          >
            <X size={13} />
          </button>
        </span>
...
function EmojiPicker({
  ...
  customEmojis?: import("./connectData").CustomEmoji[];
}) {
  ...
  useEffect(() => {
    try {
      setRecent(
        JSON.parse(window.sessionStorage.getItem(RECENT_EMOJI_KEY) || "[]"),
      );
    } catch {
      setRecent([]);
    }
  ...
  const customBlock =
    ...
    !query.trim() &&
    (category === "custom" || category === "recent")
      ? customEmojis
      : [];
  ...
  return (
    <div className={styles.s341}>
        ...
        <div className={styles.s344}>
          {[
            ...
            <button
              ...
              style={{
                background:
                  category === c.key ? "var(--color-primary-light)" : "none",
              }}
              className={styles.s345}
            >
              {c.icon}
            </button>
          ))}
          {customEmojis && customEmojis.length > 0 && (
            <button
              ...
              style={{
                background:
                  category === "custom" ? "var(--color-primary-light)" : "none",
              }}
              className={styles.s345}
            >
              <Smile size={15} />
      ...
      <div className={styles.s346}>
        {results.length === 0 && (
          <span className={styles.s347}>
            {category === "recent" && !query.trim()
      ...
      {preview.image && (
        <div className={styles.s353}>
          <img
            ...
            className={styles.s354}
          />
        </div>
      )}
      <div className={styles.s355}>
        {preview.siteName && (
          <div className={styles.s356}>{preview.siteName}</div>
        )}
        <div className={styles.s272}>{preview.title}</div>
        {preview.description && (
          <div className={styles.s357}>{preview.description}</div>
        )}
...
function PollCreator({ onClose }: { onClose: () => void }) {
  ...
  const submit = async () => {
    ...
    try {
      const activeId =
        (document.querySelector("[data-active-channel]") as HTMLElement)
    ...
    >
      ...
      >
        ...
        <button
          onClick={submit}
          disabled={
            creating || !question.trim() || options.filter(Boolean).length < 2
          }
          style={{
            background: "var(--color-primary)",
            color: "#fff",
...
function PollBlock({
  ...
  poll: import("./connectData").ConnectPoll;
  channelId: string;
  onRefresh: () => void;
}) {
  const toast = useToast();
    ...
    >
      ...
      {poll.options?.map((opt: any) => {
        ...
        return (
          <button
            key={opt.id}
            onClick={() => !poll.isClosed && vote(opt.id)}
...
function PollBlockInline({
  ...
  const [poll, setPoll] = useState<import("./connectData").ConnectPoll | null>(
    null,
  );
  const toast = useToast();
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <h3 style={{ margin: 0 }}>😃 Custom Emoji</h3>
          <button
            onClick={onClose}
            style={{ background: "none", border: "none", cursor: "pointer" }}
          >
        ...
        >
          ...
          />
          <span>
            {uploading ? "Uploading..." : "📁 Upload emoji (PNG/GIF/JPEG/WEBP)"}
          </span>
        </label>
        <div
          style={{
...
function MeetingRecap({
  ...
  const generate = async () => {
    setGenerating(true);
    try {
      const s = await api.generateMeetingSummary(meetingId);
      setSummary(s);
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <h3 style={{ margin: 0 }}>📋 Meeting Recap</h3>
          <button
            onClick={onClose}
            style={{ background: "none", border: "none", cursor: "pointer" }}
          >
...
function TemplateDialog({ onClose }: { onClose: () => void }) {
  ...
  const submit = async () => {
    ...
    setCreating(true);
    try {
      await api.createFromTemplate(selected, name);
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <h3 style={{ margin: 0 }}>📋 Channel Templates</h3>
          <button
            onClick={onClose}
            style={{ background: "none", border: "none", cursor: "pointer" }}
          >
...
function EphemeralBadge({
  ...
  message: import("./connectData").ConnectMessage;
  onView: () => void;
}) {
  const [viewed, setViewed] = useState(false);
  if (viewed || (message as any).viewOnceViewed)
    return (
      <div
        style={{
          fontStyle: "italic",
...
function TranslateButton({
  ...
  const [result, setResult] = useState<
    import("./connectData").TranslateResult | null
  >(null);
  const toast = useToast();
  const translate = async () => {
    ...
    setTranslating(true);
    try {
      const r = await api.translateMessage(messageId, "en");
      setResult(r);
    } catch {
    ...
    <div className={styles.s365}>
      {tabs.map((tab: any) => (
        <a
          key={tab.id}
          href={tab.url || "#"}
          target={tab.url ? "_blank" : undefined}
          rel="noopener"
          className={styles.s366}
        >
          {tab.icon ? (
            <span className={styles.s367}>{tab.icon}</span>
          ) : (
            <Link2 size={14} />
          )}
          <span>{tab.label}</span>
